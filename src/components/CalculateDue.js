...
function CalculateDue({ describe, inputDate }) {
    ...
    return (
        <>
            <Badge pill bg="primary">과제 설명</Badge> {describe} <br/>
            {gap < 0
            ? <><Badge pill bg="primary">제출 마감</Badge> {newYear}년 {newMonth}월 {newDate}일, {newHour}시 {newMin}분</>
            : <><Badge pill bg="secondary">제출 마감</Badge> {newYear}년 {newMonth}월 {newDate}일, {newHour}시 {newMin}분</>}
