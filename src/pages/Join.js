...
function Join() {
  ...
  const pw1Handler = (event) => {
    event.preventDefault();
    setPw1(event.target.value);
  };
  const pw2Handler = (event) => {
    event.preventDefault();
    ...
    await axios
      ...
      .then(async (result) => {
        if (result.data.code !== 0) alert('이미 존재하는 아이디입니다.');
          ...
          await axios
            .post(`${SERVER_URI}/api/user/interest`, {
              email,
              interest0,
              interest1,
              interest2,
      ...
      <div id="div-scroll-join" class="div-layout-lower-2">
        <div>
          ...
          <Form id="join-form">
            ...
            <Form.Group as={Row} class="join-row">
              ...
              <Col sm="3">
                <Form.Control
                  ...
                  required
                />
                <Form.Text>6자 이상, 20자 이하로 입력해주세요.</Form.Text>
              </Col>
            </Form.Group>
            <Form.Group as={Row} class="join-row">
              <Form.Label column sm="2">
            ...
            <br />
            <Button variant="success" id="create-btn" onClick={onClickJoin}>
              계정 생성
            </Button>
          </Form>
        </div>
